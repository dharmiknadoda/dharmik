...
if (isset($_POST['submit']) && isset($_FILES['poster'])) {
    ...
    $id = $_GET['id'];
    if (isset($_GET['id'])) {
        if(move_uploaded_file($tmp_poster,"C:/xampp/htdocs/PHP/project-4/pic/".$poster)){
           $update = "update sport set name='$name' , tour='$tour' , language='$language' , date='$date' ,  location='$location' , poster='$poster' where id ='$id'";
            $query = mysqli_query($conn, $update);
        }
    } else {
        if(move_uploaded_file($tmp_poster,"C:/xampp/htdocs/PHP/project-4/pic/".$poster)){
            $insert = "insert into sport(name,tour,language,date,location,poster) values('$name','$tour','$language','$date','$location','$poster')";
...
if (isset($_GET['filter']) && isset($_GET['filter_btn'])) {
    ...
    $sql = "SELECT * FROM `sport` WHERE `name` LIKE '%$filtername%'";
...
<body>
    ...
    <div id="main-wrapper" data-layout="vertical" data-navbarbg="skin5" data-sidebartype="full" data-sidebar-position="absolute" data-header-position="absolute" data-boxed-layout="full">
        ...
        <div class="page-wrapper" style="background-color: white;">
            ...
            <h2 class="hhh">All sport Data</h2>
                        ...
                        if ($query) {
                            while ($row = mysqli_fetch_assoc($query)) {
                                ...
                                echo '<tr>
                                    ...
                                    <td>' . $date . '</td>
                                    <td>' . $location . '</td>
                                    <td>' . '<button type="button" class="btn"><a href="event.php?delete=' . $id . '">Delete</a></button>' . '</td>
                                    <td>' . '<button type="button" class="btn btnn"><a href="event.php?id=' . $id . '">Update</a></button>' . '</td>
                                </tr>';
                            }
                        }
                        ?>
